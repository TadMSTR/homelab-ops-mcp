...
#: dropping it.
VERB_CLASSES: tuple[str, ...] = (GIT_WRITE, GH_WRITE, GIT_READ, GH_READ, VCS_OTHER, OTHER)
...
def _strip_prefixes(seg: list[str]) -> list[str]:
    ...
    while i < len(seg):
        ...
        if "=" in tok and not tok.startswith("-") and tok.split("=", 1)[0].isidentifier():
            i += 1
            continue
        if PurePosixPath(tok).name in _PREFIXES:
            i += 1
            # Skip that wrapper's own flags (and a value for the ones that take one).
            while i < len(seg) and seg[i].startswith("-"):
                takes_value = seg[i] in {"-u", "-g", "-n", "-S", "--user", "--group"}
                i += 1
...
def _classify_git(args: list[str]) -> str:
    ...
    if sub in _GIT_SUBACTION_READS:
        ...
        if action is None:
            return GIT_READ  # bare `git remote` / `git stash` list
...
def classify_verb(command: str) -> str:
    ...
    for seg in _segments(_tokenize(command)):
        ...
        if name == "git":
            found = _classify_git(seg[1:])
        elif name == "gh":
            found = _classify_gh(seg[1:])
        else:
            continue
